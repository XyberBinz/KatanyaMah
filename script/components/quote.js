(function () {
  ...
  window.Components.quote = {
    ...
    animate(tl, el) {
      ...
      tl.from(card, { duration: 1.2, opacity: 0, scale: 0.9, y: 30 })
        .from(mark, { duration: 0.8, opacity: 0, scale: 3, rotation: -20 }, "-=0.2")
        .from(text, { duration: 1.0, opacity: 0, y: 15 }, "-=0.1");

      if (author) {
        tl.from(author, { duration: 0.8, opacity: 0, x: -20 });
      }

      tl.to(card, { duration: 1.2, opacity: 0, y: -20 }, "+=4");
    },
  };
})();
